    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if not nome or not valor:
                return jsonify({'success': False, 'message': 'Nome e valor são obrigatórios'})
            
            conn.execute(
                'INSERT INTO servicos (nome, valor, imagem, usuario_id) VALUES (?, ?, ?, ?)',
                (nome, float(valor), imagem, usuario_id)
            )
            conn.commit()
            conn.close()
